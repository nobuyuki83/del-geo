...
pub fn dw_intersection<T>(
    ...
    pd: &nalgebra::Vector2<T>, // point
    qs: &nalgebra::Vector2<T>, // point
    qd: &nalgebra::Vector2<T>) -> (nalgebra::Vector2<T>, nalgebra::Matrix2<T>, nalgebra::Matrix2<T>)
    where T: nalgebra::RealField + Copy
{
    let qn = crate::vec2::rotate90(qd);
    let a = qs - ps;
    ...
    (ps + pd.scale(t), pd*dt_dqs.transpose(), pd*dt_dqd.transpose())
}
...
pub fn dw_intersection_against_bisector<Real>(
    ...
    p1: &nalgebra::Vector2<Real>) // point 1
    -> (nalgebra::Vector2<Real>, nalgebra::Matrix2<Real>, nalgebra::Matrix2<Real>)
...
fn test_dw_intersection_against_bisector() {
    ...
    let (t0, dtds0, dtdd0)
        = dw_intersection_against_bisector(&ps, &pd, &qs0, &qd0);
    let eps = 1.0e-5;
    for i_dim in 0..2 {
        let dqs = crate::vec2::basis(i_dim, 1.);
        let qs1 = qs0 + dqs.scale(eps);
