...
__device__ auto projected_spd_mat3(const float *p_mat, const float *quat0,
                                   const float *d)
    -> cuda::std::array<float, 3> {
  const cuda::std::array<float, 9> r = quaternion::to_mat3_col_major(quat0);
  const cuda::std::array<float, 6> pr =
      mat2x3_col_major::mult_mat3_col_major(p_mat, r.data());
...
/// ax^2 + 2bxy + cy^2 = 1
__device__ auto principal_directions(const float *coeff)
    -> cuda::std::tuple<cuda::std::array<float, 2>,
                        cuda::std::array<cuda::std::array<float, 2>, 2>> {
...
__device__ auto safe_inverse(const float *coeff) -> cuda::std::array<float, 3> {
  const float a = coeff[0];
  const float b = coeff[1];
  const float c = coeff[2];
...
__device__ auto safe_inverse_preserve_positive_definiteness(const float *abc,
                                                            float eps)
    -> cuda::std::array<float, 3> {
...
__device__ auto aabb2(const float *coeff) -> cuda::std::array<float, 4> {
  const float a = coeff[0];
  const float b = coeff[1];
  const float c = coeff[2];
  const float det = a * c - b * b;
...
__device__ float mult_vec_from_both_sides(const float *m, const float *b,
                                          const float *c) {
  return m[0] * b[0] * c[0] + m[1] * (b[0] * c[1] + b[1] * c[0]) +
         m[2] * b[1] * c[1];
}
