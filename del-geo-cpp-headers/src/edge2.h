...
__device__ float length(const float *ps, const float *pe) {
  float x = pe[0] - ps[0];
  float y = pe[1] - ps[1];
  return sqrt(x * x + y * y);
}

__device__ auto unit_edge_vector(const float *ps, const float *pe)
...
__device__ auto intersection_edge2(const float *s0, const float *e0,
                                   const float *s1, const float *e1)
    -> cuda::std::optional<RatioOfEdgeIntersection> {
...
__device__ auto dldw_intersection_edge2(const float *s0, const float *e0,
                                        const float *s1, const float *e1,
                                        float dldr0, float dldr1)
  ...
  return BackwardIntersectionEdge2{
      vec2::add_three(get<0>(s0e0s1_1).data(), get<0>(s0e0e1_2).data(),
                      get<2>(s1e1s0_3).data()),
      vec2::add_three(get<1>(s0e0s1_1).data(), get<1>(s0e0e1_2).data(),
                      get<2>(s1e1e0_4).data()),
      vec2::add_three(get<2>(s0e0s1_1).data(), get<0>(s1e1s0_3).data(),
                      get<0>(s1e1e0_4).data()),
      vec2::add_three(get<2>(s0e0e1_2).data(), get<1>(s1e1s0_3).data(),
                      get<1>(s1e1e0_4).data())};
  /*
