...
__device__ auto normal(const float *v1, const float *v2, const float *v3)
...
__device__ auto dw_normal(const float *p0, const float *p1, const float *p2)
    -> DwNormal {
...
__device__ auto intersection_against_line_bwd_wrt_tri(
    const float *p0, const float *p1, const float *p2, const float *org,
    const float *dir, float d_t, float d_u, float d_v)
    -> cuda::std::optional<IntersectionAgainstLineBwdWrtTri> {
  ...
  auto d_r = vec3::add(vec3::scale(e2.data(), d_r_dot_e2).data(),
                       vec3::scale(e1.data(), d_r_dot_e1).data());
  //
  ...
  //
  const auto d_p0 =
      vec3::add(vec3::sub(d_e1.data(), d_e2.data()).data(), d_c.data());
