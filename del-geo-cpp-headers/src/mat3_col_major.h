...
__device__ auto from_vec3_to_skew_mat(const float *v)
    -> cuda::std::array<float, 9> {
  return {0., v[2], -v[1], -v[2], 0., v[0], v[1], -v[0], 0.};
}

__device__ auto mult_mat(const float *a, const float *b)
    -> cuda::std::array<float, 9> {
...
__device__ auto mult_vec(const float *a, const float *b)
...
__device__ auto try_inverse(const float *b)
    -> cuda::std::optional<cuda::std::array<float, 9>> {
  const float det = b[0] * b[4] * b[8] + b[3] * b[7] * b[2] +
                    b[6] * b[1] * b[5] - b[0] * b[7] * b[5] -
                    b[6] * b[4] * b[2] - b[3] * b[1] * b[8];
  if (det == 0.f) {
    return {};
  }
...
__device__ auto to_mat2x3_col_major_xy(const float *m)
